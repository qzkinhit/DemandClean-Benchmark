#!/bin/bash
# ============================================================
# Horizon Baseline 完整测评脚本
# 注意: Horizon需要FD规则文件（在各数据集的rules.txt中定义）
...
for dataset_config in "${ALL_DATASETS[@]}"; do
    ...
    [[ ! -f "$rule_path" ]] && echo "跳过: $rule_path 不存在" && failed=$((failed + 1)) && continue
...
echo "============================================================"
echo "Horizon 测评完成: 总计=$total 成功=$success 失败=$failed"
echo "============================================================"
