...
MLImputer是基于机器学习的缺失值填充方法。

特点:
- 支持MICE、KNN、随机森林等插补策略
- 全自动执行 (Type 1)
- 适用于缺失值填充任务

用法:
    python run_mlimputer_base.py --dirty_path <脏数据路径> --clean_path <干净数据路径>

示例:
    python run_mlimputer_base.py \\
        --dirty_path ../../Data/hospital/dirty_index.csv \\
        --clean_path ../../Data/hospital/clean_index.csv \\
        --task_name hospital_mlimputer \\
        --method mice \\
...
from Methods.MLImputer.mlimputer_wrapper import MLImputerWrapper


def main():
    parser = argparse.ArgumentParser(
        description='Run MLImputer data cleaning.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python run_mlimputer_base.py --dirty_path ../../Data/hospital/dirty.csv --clean_path ../../Data/hospital/clean.csv

支持的插补方法:
  - mice: Multiple Imputation by Chained Equations
  - knn: K-Nearest Neighbors插补
  - rf: 随机森林插补
        """
    )
    ...
    # 任务参数
    parser.add_argument('--task_name', type=str, default='beers_mlimputer',
                        help='任务名称')
    parser.add_argument('--output_path', type=str, default='../../results/mlimputer/',
                        help='结果输出路径')
    ...
    if success and os.path.exists(args.clean_path):
        ...
        try:
            ...
            eval_results = run_all_evaluation(
                ...
                models=args.models,
                method_type=1,  # MLImputer是全自动Type 1
                ground_truth_used=0,
