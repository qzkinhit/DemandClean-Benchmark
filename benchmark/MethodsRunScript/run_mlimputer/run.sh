#!/bin/bash
# ============================================================
# MLImputer Baseline full benchmark script
...
echo "============================================================"
echo "MLImputer Baseline full benchmark"
...
mkdir -p logs/mlimputer
mkdir -p results/mlimputer
...
for dataset_config in "${ALL_DATASETS[@]}"; do
    ...
    task_name="${dataset}_mlimputer${VERSION:+_$VERSION}"
    log_file="logs/mlimputer/${task_name}.log"
    ...
    cmd="python MethodsRunScript/run_mlimputer/run_mlimputer_base.py \
        --dirty_path $dirty_path --clean_path $clean_path \
        --task_name $task_name --output_path results/mlimputer/ \
        --index_attribute index --label_column $label_column \
        --task_type $task_type --models $models --method $METHOD --verbose"
...
echo "============================================================"
echo "MLImputer benchmark done: total=$total success=$success failure=$failed"
echo "============================================================"
