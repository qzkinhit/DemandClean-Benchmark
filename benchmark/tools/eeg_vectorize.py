...
def extract_features_and_labels(df):
    # Use the first 14 EEG channels directly as features.
    eeg_features = df.iloc[:, :14]  # first 14 columns are EEG signals

    # The eyeDetection label column, cast to int.
...
def save_to_csv(features, labels, output_path):
    ...
    standardized_data['eyeDetection'] = labels  # append labels to the standardized data
    standardized_data.to_csv(output_path, index=False)
    print(f"Standardized data saved to: {output_path}")

# Main: load ARFF, standardize, and save.
